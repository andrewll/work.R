cycletime <- function(file1){
  ...
  dat2 <- subset(dat, select=c("EG","DeliveryNumber","ProjectTitle","RTEGOTDF","RTEGActualDeliveryDate", "ProjectCategory", "OrderCOnfirmtoDock_Actual", "DTR1", "OrderCOnfirmtoRTEG_Actual2","ProjectCreationDate","woadPOCreation","woadDock2"))
  
  ## add calculated variables
  dat3 <- mutate(dat2, Month_Delivered = format(mdy(RTEGActualDeliveryDate),"%b"), Year_Delivered = format(mdy(RTEGActualDeliveryDate),"%Y"), PIDCount = 1, pidcreate = mdy(ProjectCreationDate), pocreate = mdy(woadPOCreation), dockdate = mdy(woadDock2))
  dat4 <- mutate(dat3, pidcreate_to_pocreate = as.numeric((pocreate - pidcreate) / 86400), pocreate_to_dock = as.numeric((dockdate - pocreate)/86400))
  ...
  write.csv(dat5,file="ouput_cycletime.csv")
  View(dat5)
